  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <h1 className="font-serif text-3xl tracking-tight text-ink">Clientes</h1>
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          title={query ? "Sin resultados" : "Todavía no tienes clientes"}
          description={query ? "Prueba con otro término." : "Añade un cliente para asociarlo a tus proyectos y presupuestos."}
          action={!query && <Button onClick={() => setEditing("new")}>Agregar cliente</Button>}
        />
      ) : (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {clients.map((c) => (
            <div key={c.id} className="rounded-2xl border border-line bg-surface p-5">
              ...
              <p className="mt-3 text-xs text-muted">{counts[c.id] ?? 0} proyecto(s)</p>
            </div>
          ))}
        </div>
      )}
