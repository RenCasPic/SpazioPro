  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h2 className="font-serif text-2xl text-ink">Imágenes · Antes / Después</h2>
        {original && (
          <div className="flex items-center gap-2">
            ...
            <div className="flex rounded-full border border-line-strong p-0.5">
              ...
              <Toggle active={mode === "split"} onClick={() => setMode("split")} icon={<Rows2 className="h-4 w-4" />} label="Lado a lado" />
            </div>
          </div>
        )}
      </div>
      ...
      {!original || replacing ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <figure>
            ...
            <img src={original.originalUrl} alt="Original" className="aspect-[4/3] w-full rounded-2xl border border-line-strong object-cover" />
            <figcaption className="mt-2 text-center text-xs text-muted">Estado actual</figcaption>
          </figure>
          <figure>
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img
            ...
            />
            <figcaption className="mt-2 text-center text-xs text-muted">Propuesta de diseño</figcaption>
          </figure>
        </div>
      )}

      <p className="text-xs text-muted">
        En modo demo la propuesta se representa como un ajuste de color sobre la foto original. Al
