...
function swatch(bundle: ProjectBundle, surface: SurfaceKind): string | null {
  const item = bundle.items.find(
    (i) => i.scenarioId === bundle.project.activeScenarioId && i.kind === "surface" && i.surface === surface,
  );
...
export function SurfacePreview({ bundle, className }: { bundle: ProjectBundle; className?: string }) {
  ...
  return (
    <div className={cn("relative overflow-hidden bg-paper", className)}>
      ...
      <div className="absolute left-0 right-0 top-0" style={{ height: "26%", background: ceiling, clipPath: "polygon(0 0, 100% 0, 72% 100%, 28% 100%)" }} />
      <div className="absolute bottom-0 left-0 right-0" style={{ height: "42%", background: floor, clipPath: "polygon(28% 0, 72% 0, 100% 100%, 0 100%)" }} />
