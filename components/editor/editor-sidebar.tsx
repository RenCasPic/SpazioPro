...
import {
  PaintBucket, Sofa, ChefHat, Bath, Lightbulb, Flower2, Hammer,
  Search, SlidersHorizontal, Plus, Wand2, AlertTriangle,
} from "lucide-react";
import type { ProductGroup } from "@/types";
import { CATEGORIES, GROUP_LABELS, GROUP_ORDER, categoryMeta } from "@/data/categories";
import { SURFACE_LABELS } from "@/lib/constants";
...
export function EditorSidebar({ bundle }: { bundle: ProjectBundle }) {
  ...
  const country = bundle.project.countryCode;
  ...
  const entries = useCatalog(
    useMemo(
      ...
      [country, group, category, query, brand, style, maxPrice],
    ),
  );
  ...
  async function apply(entry: (typeof entries)[number]) {
    ...
    if (meta.kind === "surface") {
      ...
      push(`"${entry.product.name}" aplicado. Vista previa demo generada.`, "success");
    } else {
      await addProduct(entry.product);
      push(`"${entry.product.name}" añadido a la escena.`, "success");
    }
  }
      ...
      <div className="flex min-w-0 flex-1 flex-col bg-surface">
        <div className="border-b border-line px-4 pb-3 pt-3.5">
            ...
            <div className="mt-3 space-y-2.5 rounded-xl bg-paper p-3">
              ...
              <div>
                <div className="flex justify-between text-[11px] text-muted">
                  ...
                  <span>
                    {maxPrice
                      ? currencyService.formatFor(maxPrice, country)
                      : "Sin límite"}
                  </span>
                </div>
                <input
          ...
          {activeSurface && (
            <p className="mt-2.5 flex items-center gap-1.5 rounded-lg bg-clay-tint px-2.5 py-1.5 text-[11px] text-clay-dark">
              ...
              Zona activa: {SURFACE_LABELS[activeSurface]}.
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-3">
          <div className="grid gap-2.5">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-0.5 flex items-center gap-1 text-[11px]">
                      <span
                        className={cn(
                          "rounded px-1 py-px",
                          price.source === "converted" || price.source === "missing"
                    ...
                    <div className="mt-1.5 flex items-center justify-between">
                      <span className="text-sm font-semibold text-ink">
                        {currencyService.format(price.money, bundle.project.locale)}
                        <span className="text-[11px] font-normal text-muted"> /{UNIT_LABELS[product.unit]}</span>
                      </span>
                      <button
                        onClick={() => apply(entry)}
            ...
            {entries.length === 0 && <p className="py-10 text-center text-sm text-muted">No hay productos con esos filtros.</p>}
          </div>
        </div>
      </div>
    </div>
