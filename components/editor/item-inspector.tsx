...
export function ItemInspector({ item, bundle }: { item: ProjectItem; bundle: ProjectBundle }) {
  ...
  const money = (n: number) =>
    currencyService.format({ amount: n, currency: item.currencyCode }, bundle.project.locale);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-2">
        <Field label={`Precio (${UNIT_LABELS[item.unit]})`}>
          <Input
            type="number"
            step="0.01"
            min="0"
            ...
            key={item.unitPrice}
            onBlur={(e) => updateItem(item.id, { unitPrice: Number(e.target.value), priceSource: "market" })}
          />
        </Field>
        <Field label={`Mano de obra (${UNIT_LABELS[item.unit]})`}>
          <Input
            ...
            key={item.laborCost}
            onBlur={(e) => updateItem(item.id, { laborCost: Number(e.target.value) })}
          />
        </Field>
      </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          <Field label="Desperdicio (%)">
            <Input
              ...
              key={item.wastePercent}
              onBlur={(e) => updateItem(item.id, { wastePercent: Number(e.target.value), quantityAuto: true })}
            />
          </Field>
          <Field label={`Cantidad (${UNIT_LABELS[item.unit]})`}>
            <Input
              ...
              step="0.01"
              min="0"
              defaultValue={formatNumber(resolveItemQuantity(item, dims))}
              key={item.quantity + String(item.quantityAuto)}
              onBlur={(e) => updateItem(item.id, { quantity: Number(e.target.value) })}
            />
          </Field>
          {item.quantityAuto && (
            <p className="col-span-2 text-[11px] text-muted">
              Cantidad automática desde las dimensiones + {item.wastePercent}% de desperdicio.
            </p>
          )}
        </div>
      ) : (
        <Field label="Cantidad">
          <div className="flex items-center gap-2">
            ...
            <Input
              ...
              key={item.quantity}
              onBlur={(e) => updateItem(item.id, { quantity: Math.max(1, Number(e.target.value)) })}
            />
            <button
      ...
      {!isSurface && (
        <div className="space-y-2 rounded-xl bg-paper p-3">
          <Slider
            ...
            max={180}
            step={1}
            onChange={(v) => setTransform(item.id, { rotation: v })}
          ...
          <Slider
            ...
            max={400}
            step={5}
            onChange={(v) => setTransform(item.id, { scale: v / 100 })}
            onCommit={commitTransform}
          />
        </div>
      )}
