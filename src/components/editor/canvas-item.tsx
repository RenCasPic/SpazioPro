...
interface Props {
  ...
  selected: boolean;
  onSelect: () => void;
  onTransform: (patch: Partial<ProjectItem["transform"]>) => void;
  onCommit: () => void;
...
export function CanvasItem({ item, selected, onSelect, onTransform, onCommit, containerRef }: Props) {
  ...
  function onPointerDown(e: React.PointerEvent, m: DragMode) {
    ...
    start.current = {
      ...
      scale: t.scale,
      rotation: t.rotation,
      cx: r.left + (t.x / 100) * r.width,
      cy: r.top + (t.y / 100) * r.height,
    };
  }
  ...
  function onPointerMove(e: React.PointerEvent) {
    ...
    if (mode.current === "move") {
      const dx = ((e.clientX - s.px) / r.width) * 100;
      const dy = ((e.clientY - s.py) / r.height) * 100;
      onTransform({
        x: clamp(s.x + dx, 2, 98),
        y: clamp(s.y + dy, 2, 98),
      });
  ...
  return (
    ...
    >
      <div
        style={{ transform: `rotate(${t.rotation}deg) scale(${t.scale})` }}
        className="relative"
      >
        <button
          ...
          className={cn(
            "grid h-16 w-16 cursor-move place-items-center rounded-2xl border bg-surface/95 text-3xl shadow-[var(--shadow-pop)] backdrop-blur transition-colors",
