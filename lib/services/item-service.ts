...
export const itemService = {
  async add(params: {
    ...
    const price = pricingService.resolve(params.product.id, p.countryCode);
    const laborRate = laborRateService.rate(p.countryCode, params.product.laborCategory);
  ...
  /** Copy every item from one scenario to another (for scenario branching). */
  async cloneScenarioItems(projectId: string, fromScenarioId: string, toScenarioId: string): Promise<void> {
    mutateDb((db) => {
      const source = db.items.filter(
        (i) => i.projectId === projectId && i.scenarioId === fromScenarioId,
      );
